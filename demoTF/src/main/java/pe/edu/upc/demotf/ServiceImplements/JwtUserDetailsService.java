...
public class JwtUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        Users user=repo.findByNameUser(username);
...
//user.getUsername(), user.getPassword(), user.getEnabled(), true, true, true, roles
        UserDetails ud = new org.springframework.security.core.userdetails.User(user.getNameUser(),user.getPassword(),user.getEnabled(),true,true,true,roles);
        return ud;
    }
